...
def calculate_normals(points):
    ...
    safe_seg_norm_magnitudes = np.where(
        is_seg_norm_effectively_zero, 1.0, seg_norm_magnitudes
    )
    ...
    safe_vert_norm_magnitudes = np.where(
        is_vert_norm_effectively_zero, 1.0, vert_norm_magnitudes
    )
...
def create_ellipse_points(
    num_points, center_x=0, center_y=0, radius_x=1, radius_y=1, rotation_angle_rad=0
):
