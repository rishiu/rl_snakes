...
from utils import calculate_normals

def visualize_snake_evolution(image, snake_coords_list, title="Snake Evolution", normal_scale=5.0):
    ...
    quiver_normals = ax.quiver(initial_snake_points[0,:], initial_snake_points[1,:],
                               initial_normals[0,:], initial_normals[1,:],
                               color='blue', units='xy', scale=1.0/normal_scale, headwidth=3, headlength=4, width=0.002*min(image.shape))
    ...
    def update(val):
        ...
        quiver_normals.set_offsets(np.c_[current_snake_points[0,:], current_snake_points[1,:]])
...
def plot_snake_on_image(image, snake_points, title="Snake on Image", normal_scale=5.0):
    ...
    if snake_points is not None and snake_points.shape[0] == 2 and snake_points.shape[1] > 0:
        ...
        if vertex_normals.shape[1] > 0:
            ax.quiver(snake_points[0,:], snake_points[1,:],
                      vertex_normals[0,:], vertex_normals[1,:],
                      color='blue', units='xy', scale=1.0/normal_scale, headwidth=3, headlength=4, width=0.002*min(image.shape), zorder=2)
            ax.quiver(segment_midpoints[0,:], segment_midpoints[1,:], 
                      segment_normals[0,:], segment_normals[1,:],
                      color='green', units='xy', scale=1.0/normal_scale, headwidth=3, headlength=4, width=0.002*min(image.shape), zorder=2)
    else:
...
def visualize_snake_evolution_with_energy_rewards(image, snake_coords_list, rewards_list, internal_energy_list=None, external_energy_list=None, title="Snake Evolution with Metrics", normal_scale=5.0):
    if not snake_coords_list:
        print("No snake coordinates to visualize.")
        plt.imshow(image, cmap='gray')
        plt.title("Image (No Snake Data)")
    ...
    ax_snake = fig.add_subplot(gs[0, :]) # Snake plot spans all columns in the first row
    ax_reward = fig.add_subplot(gs[1, 0])
    ax_internal_e = fig.add_subplot(gs[1, 1])
    ax_external_e = fig.add_subplot(gs[1, 2])
    ax_slider_area = fig.add_subplot(gs[2, :]) # Slider spans all columns in the last row
    ...
    quiver_normals = ax_snake.quiver(initial_snake_points[0,:], initial_snake_points[1,:],
                               initial_normals[0,:], initial_normals[1,:],
                               color='blue', units='xy', scale=1.0/normal_scale, headwidth=3, headlength=4, width=0.002*min(image.shape[:2]))
    ...
    if internal_energy_list:
        ax_internal_e.plot(iterations, internal_energy_list, label='Internal Energy', color='purple')
    ...
    if external_energy_list:
        ax_external_e.plot(iterations, external_energy_list, label='External Energy', color='orange')
    ...
    def update(val):
        ...
        if vline_reward: vline_reward.set_xdata([iteration])
        if vline_internal_e: vline_internal_e.set_xdata([iteration])
        if vline_external_e: vline_external_e.set_xdata([iteration])
    ...
    fig.tight_layout(rect=[0, 0.03, 1, 0.97]) # Adjust rect to ensure suptitle and slider are visible
    plt.show()