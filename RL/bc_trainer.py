...
class BCTrainer:
    ...
    def select_action(self, state):
        ...
        with torch.no_grad():
            if self.model_type == 'mlp':
                ...
                action, action_logprob, state_val = self.policy.act(image_tensor_dev, vector_tensor_dev)
    ...
    def add_reward_and_done(self, reward, done):
        ...
        if self.get_gt_action is None:
            raise ValueError("Ground truth action function not set. Call set_gt_action_fn() first.")
    ...
    def update(self, last_value_of_episode=None):
        ...
        # Prepare batch data
        if self.model_type == 'mlp':
            states_batched = torch.stack([s for s in self.buffer.states]).to(self.device).squeeze(1)
            ...
            images_batched = torch.cat([s[0] for s in self.buffer.states], dim=0).to(self.device)
            vectors_batched = torch.cat([s[1] for s in self.buffer.states], dim=0).to(self.device)
        ...
        for _ in range(self.K_epochs):
            if self.model_type == 'mlp':
                ...
                shared_features = self.policy.forward_features(images_batched, vectors_batched)
