...
class RolloutBuffer:
    ...
    def add_pending_transition(self, state, action, logprob, state_value):
        ...
        self.state_values.append(
            state_value
        )  # state_value is already a tensor e.g. (1,)
    ...
    def update_last_transition(self, reward, is_terminal):
        ...
        if not self.rewards or self.rewards[-1] is not None:
            print(
                "Warning: Attempting to update a non-pending or non-existent transition."
            )
    ...
    def compute_returns_and_advantages(self, last_value, gamma, gae_lambda):
        ...
        if not self.state_values:
            ...
            state_values_tensor = torch.stack(
                [sv.float().reshape(-1) for sv in self.state_values]
            ).squeeze()
        ...
        for t in reversed(range(num_steps)):
            if t == num_steps - 1:
                ...
                next_value = state_values_tensor[
                    t + 1
                ]  # state_values_tensor is (num_steps,)

            delta = (
                rewards[t]
                + gamma * next_value * next_non_terminal
                - state_values_tensor[t]
            )
...
class CNNActorCritic(nn.Module):
    def __init__(
        self, vector_input_dim, action_dim, action_std_init=0.1, image_channels=1
    ):
        ...
        self.cnn_base = nn.Sequential(
            nn.Conv2d(
                image_channels, 16, kernel_size=5, stride=2, padding=2
            ),  # Output: 16x100x100
            nn.ReLU(),
        ...
        with torch.no_grad():
            ...
            cnn_flat_dim = self.cnn_base(dummy_img).shape[
                1
            ]  # Should be 32 * 12 * 12 = 4608
        ...
        self.vector_feature_processor = nn.Sequential(
            nn.Linear(vector_input_dim, 128), nn.ReLU(), nn.Linear(128, 64), nn.ReLU()
        )

        combined_features_dim = (
            256 + 64
        )  # From image_feature_processor and vector_feature_processor

        # Shared layers after concatenation
        self.shared_layers = nn.Sequential(
            nn.Linear(combined_features_dim, 128), nn.ReLU()
        )
        ...
        # Critic head
        self.critic_head = nn.Sequential(nn.Linear(128, 1))
    ...
    def act(self, image_input, vector_input):
        ...
        action_logprob = dist.log_prob(action).sum(
            axis=-1, keepdim=True
        )  # Sum across action dimensions, keep batch dim
    ...
    def evaluate(self, image_input, vector_input, action):
        ...
        action_logprobs = dist.log_prob(action).sum(
            axis=-1
        )  # Sum across action dimensions -> (N,)
...
class PPOTrainer:
        ...
        if self.model_type == "mlp":
            ...
            optimizer_params = [
                ...
                {"params": self.policy.critic.parameters(), "lr": critic_lr},
                {
                    ...
                    "lr": actor_lr * 0.1,
                },  # Ensure action_log_std is in a list
            ]
    ...
    def select_action(self, state):
        with torch.no_grad():
            if self.model_type == "mlp":
                ...
                image_input, vector_input = (
                    state  # Each is (1, C, H, W) and (1, vec_dim) respectively
                )
        ...
        self.buffer.add_pending_transition(
            state_to_buffer,
            action.cpu(),
            action_logprob.cpu(),
            state_val.cpu().squeeze(),
        )
        return (
            action.cpu().numpy().squeeze(0)
        )  # Return numpy array (action_dim,) for environment
    ...
    def update(self, last_value_of_episode):
        ...
        if self.model_type == "mlp":
            ...
            old_states_batched = (
                torch.stack([s for s in self.buffer.states])
                .to(self.device)
                .detach()
                .squeeze(1)
            )
            if (
                old_states_batched.ndim == 1 and len(self.buffer.states) == 1
            ):  # case of single step in buffer, stack might not give (1, dim)
            ...
            old_images_batched = (
                torch.cat([s[0] for s in self.buffer.states], dim=0)
                .to(self.device)
                .detach()
            )
            old_vectors_batched = (
                torch.cat([s[1] for s in self.buffer.states], dim=0)
                .to(self.device)
                .detach()
            )
        else:
        ...
        if (
            old_actions.ndim == 3 and old_actions.shape[1] == 1
        ):  # (N, 1, action_dim) -> (N, action_dim)
            old_actions = old_actions.squeeze(1)
        elif (
            old_actions.ndim == 2
            and old_actions.shape[0] == len(self.buffer.states)
            and old_actions.shape[1] == self.policy.action_dim
        ):  # (N, action_dim)
        ...
        elif old_logprobs.ndim == 1 and old_logprobs.shape[0] == len(
            self.buffer.states
        ):
        ...
        for _ in range(self.K_epochs):
            if self.model_type == "mlp":
                logprobs, state_values, dist_entropy = self.policy.evaluate(
                    old_states_batched, old_actions
                )
            elif self.model_type == "cnn":
                logprobs, state_values, dist_entropy = self.policy.evaluate(
                    old_images_batched, old_vectors_batched, old_actions
                )
            else:
                raise ValueError(
                    f"Unknown model_type during policy evaluation: {self.model_type}"
                )
    ...
    def get_value(self, state):
        with torch.no_grad():
            if self.model_type == "mlp":
                ...
                _, _, state_value_full = self.policy_old.act(
                    image_tensor_dev, vector_tensor_dev
                )
            else:
                raise ValueError(f"Unknown model_type: {self.model_type}")
